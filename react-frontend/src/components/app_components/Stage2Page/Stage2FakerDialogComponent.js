...
import stage2FakerFactory from "./stage2FakerFactory";
...
const Stage2FakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [calonIds, setcalonIds] = useState();
const [PelulusIds, setPelulusIds] = useState();
    ...
    useEffect(() => {
        ...
        client.service("users")
                        ...
                        .then((data) => {
                        setcalonIds(data.data);
                        });
client.service("users")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        setPelulusIds(data.data);
                        });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = stage2FakerFactory(props.user,fakerCount,calonIds,PelulusIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("stage2").create(elem));
...
export default connect(mapState, mapDispatch)(Stage2FakerDialogComponent);
