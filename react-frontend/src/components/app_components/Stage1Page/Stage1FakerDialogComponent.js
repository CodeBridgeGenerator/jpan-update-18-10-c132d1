...
import stage1FakerFactory from "./stage1FakerFactory";
...
const Stage1FakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = stage1FakerFactory(props.user,fakerCount,pelulusIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("stage1").create(elem));
...
export default connect(mapState, mapDispatch)(Stage1FakerDialogComponent);
